...
from beer import Beer, BeerAnnotation,BeerData_Sentence,Beer_decorrelated
...
def parse():
    parser = argparse.ArgumentParser(description="PRAT")
...
if args.data_type=='beer':       #beer
    ...
    sentence_data = BeerData_Sentence(args.data_dir, args.aspect, 'train', word2idx, balance=True)
...
# skew_opt = torch.optim.Adam(model.generator.parameters(), lr=args.skew_lr)  # 诱导生成器
# skew_opt = torch.optim.Adam(model.get_cls_param(), lr=args.skew_lr)  # 诱导预测器
