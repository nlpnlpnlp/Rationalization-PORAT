...
class Beer(Dataset):
    ...
    def _create_examples(self, aspect, balance=False):
        ...
        # print('Dataset: Corr Beer Review')
...
'''decorrelated experiments'''
class Beer_decorrelated(Dataset):
    def __init__(self, data_dir, aspect, mode, word2idx, balance=False, max_length=256, neg_thres=0.4, pos_thres=0.6,
        ...
        with gzip.open(self.input_file, "rt") as f:
            ...
            for (i, line) in enumerate(lines):
                ...
                if labels[aspect] <= self.neg_thres:
                    ...
                    continue
                examples.append({'text': text, "label": label})
        print('Dataset: DeCorr Beer Review')
        print('{} samples has {}'.format(self.mode_to_name[self.mode], len(examples)))
...
class BeerData_Sentence(Dataset):
    def __init__(self, data_dir, aspect, mode, word2idx, balance=False, max_length=256, neg_thres=0.4, pos_thres=0.6,
                 stem='reviews.aspect{}.{}.txt.gz'):
        super().__init__()
        self.mode_to_name = {'train': 'train', 'dev': 'heldout'}
...
'''case use'''
class BeerAnnotation_Input(Dataset):
    
    def __init__(self, annotation_path, aspect, max_length=256, neg_thres=0.4, pos_thres=0.6):
        super().__init__()
        self.inputs = []
        self.masks = []
        self.labels = []
...
if __name__=="__main__":
    ...
    all_data = BeerAnnotation(annotation_path, aspect, word2idx)
...
'''
BeerData as0: The sparsity is 0.18514230847358704
BeerData as1: The sparsity is 0.15579645335674286
BeerData as2: The sparsity is 0.124354287981987
'''
