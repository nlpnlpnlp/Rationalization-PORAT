...
import time
import torch
from beer import Beer, BeerAnnotation,Beer_decorrelated
...
from train_util import train_porat
from validate_util import validate_share, validate_dev_sentence, validate_annotation_sentence, validate_rationales, validate_share_write_best_prediction
...
def parse():
    ...
    parser.add_argument('--root_save_dir',
                        type=str,
                        default="./results_final/tmp/",
                        help='reward_mode')
...
if args.data_type=='beer':   
    ...
    if args.aspect==0:
        model_pt = torch.load("./trained_embedding/hotel/pretrain_aspect0.pkl",map_location=torch.device("cuda:{}".format(args.gpu)))
        model_pt.to(device)
    elif args.aspect==1:
        model_pt = torch.load("./trained_embedding/hotel/pretrain_aspect1.pkl",map_location=torch.device("cuda:{}".format(args.gpu)))
        model_pt.to(device)
    elif args.aspect==2:
        model_pt = torch.load("./trained_embedding/hotel/pretrain_aspect2.pkl",map_location=torch.device("cuda:{}".format(args.gpu)))
        model_pt.to(device)
...
for epoch in range(args.epochs):
    start = time.time()
    model.train()
    precision, recall, f1_score, accuracy = train_porat(model, model_pt,optimizer, train_loader, device, args,(writer,epoch),grad,grad_loss,freezing=args.freezing)
    end = time.time()
    ...
    with torch.no_grad():
        ...
        print("dev epoch:{} recall:{:.4f} precision:{:.4f} f1-score:{:.4f} accuracy:{:.4f}".format(epoch, recall,precision,f1_score, accuracy))
        ...
        print("The annotation performance: sparsity: %.4f, precision: %.4f, recall: %.4f, f1: %.4f"
                % (100 * annotation_results[0], 100 * annotation_results[1],100 * annotation_results[2], 100 * annotation_results[3]))
